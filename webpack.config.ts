...
const DIST = EVENT.includes('dist');
const GITHUB = EVENT.includes('github');
...
const clientConfig = function webpackConfig(): WebpackConfig {
  ...
  if (!DLL) {
    config.output = {
      path: GITHUB ? root('docs') : root('dist/client'),
